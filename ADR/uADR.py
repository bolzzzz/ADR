    ...
    def cross_check(self, collection_of_candidates):
        ...
        for i in range(num_candidates):
            for j in range(num_candidates):
                df_cross_check.loc[i,j] = validate_sample(collection_of_candidates[i], ns_collection_of_candidates[j], 1e-10)
        ...
        ns_hof = [ns_collection_of_candidates[i] for i in range(num_candidates) if df_num_cross_check[i] < thre_numKilled]
        return hof, ns_hof
