...
def load_hdfs_structured_logs(log_path, labels_path=None):
    ...
    df_logs["BlockId"] = df_logs.apply(lambda row: re.findall(r'blk_-?\d+', row['Content'])[0], axis=1)
    if labels_path is not None:
        df_labels = pd.read_csv(labels_path, sep=',', header=0, index_col=0)
        df_logs_bLabels = df_logs.apply(lambda row: 1 if df_labels.loc[row['BlockId'], 'Label'] == "Anomaly" else 0, axis=1).values
